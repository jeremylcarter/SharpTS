...
**Last Updated:** 2026-01-31 (Added http module - interpreter only: createServer, Server, IncomingMessage, ServerResponse)
...
| `http`           | ⚠️     | Interpreter only: createServer, Server.listen/close/address, IncomingMessage, ServerResponse                                                                                                                     |
...
| `inherits()`                 | ✅     | Set up prototype chain (sets super\_)              |
| **TextEncoder**              |        |                                                    |
...
## 15. HTTP (Interpreter Only)

> **Note:** HTTP is only supported in interpreter mode. Compilation throws `NotSupportedException`.

| Feature                           | Status | Notes                           |
| --------------------------------- | ------ | ------------------------------- |
| **Server Creation**               |        |                                 |
| `http.createServer()`             | ✅     | Create HTTP server              |
| `http.createServer(callback)`     | ✅     | With request handler            |
| **Server Methods**                |        |                                 |
| `server.listen(port, callback)`   | ✅     | Start listening                 |
| `server.close(callback)`          | ✅     | Stop server                     |
| `server.address()`                | ✅     | Get bound address info          |
| **Server Events**                 |        |                                 |
| `server.on('request', handler)`   | ✅     | Request event                   |
| `server.on('listening', handler)` | ✅     | Listening event                 |
| `server.on('close', handler)`     | ✅     | Close event                     |
...
| `res.statusCode`                  | ✅     | Get/set status code             |
| `res.statusMessage`               | ✅     | Get/set status message          |
...
| `res.headersSent`                 | ✅     | Whether headers were sent       |
| `res.finished`                    | ✅     | Whether response is finished    |
| `res.req`                         | ✅     | Reference to request            |
| **Constants**                     |        |                                 |
| `http.METHODS`                    | ✅     | Array of HTTP methods           |
| `http.STATUS_CODES`               | ✅     | Status code descriptions        |
| **Not Implemented**               |        |                                 |
| `http.request()`                  | ❌     | Client requests                 |
| `http.get()`                      | ❌     | Client GET requests             |
| Request body streaming            | ❌     | Full body buffering only        |
| HTTPS                             | ❌     | Use http module only            |
